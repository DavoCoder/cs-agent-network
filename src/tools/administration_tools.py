...
def get_a2a_admin_agent_key_from_config() -> str | None:
    ...
    if not auth_user:
        logger.warning("No auth user in config, falling back to env variable")
        return os.getenv("A2A_ADMIN_AGENT_KEY")

    # Get the A2A key from auth user - handle both dict and object cases
    # When running from Studio, auth_user is a StudioUser object, not a dict
    if isinstance(auth_user, dict):
        a2a_key = auth_user.get("a2a_admin_agent_key")
    else:
        # Handle object case (e.g., StudioUser)
        a2a_key = getattr(auth_user, "a2a_admin_agent_key", None)
...
async def call_external_admin_a2a_agent(query: str) -> str:
    ...
    **Important:** Always use this tool for administrative tasks. The external agent will:
    ...
    logger.info(
        "Using A2A API key from: %s", "auth config" if _current_runtime_config else "environment"
    )
    ...
    async with httpx.AsyncClient(timeout=timeout) as httpx_client:
        ...
        try:
            ...
            if _public_card.supports_authenticated_extended_card:
                try:
                    ...
                    auth_headers_dict = {"Authorization": f"Bearer {api_key}"}
        ...
        logger.info(
            "A2A agent response extracted: %s",
