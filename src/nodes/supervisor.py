...
def classify_ticket_with_llm(
...
) -> Command[Literal["technical", "billing", "administration", END]]:
    ...
    if conversation_history:
        classification_content = (
            ...
            if user_message
            else "Current customer message: (empty or unreadable)"
        )
    else:
        classification_content = (
            f"Customer message: {user_message}"
            if user_message
            else "Customer message: (empty or unreadable)"
        )
    ...
    if classification.category == "unclassifiable":
        ...
        return Command(update=state_updates, goto=END)
