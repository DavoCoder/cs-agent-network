...
def process_assessment(
    ...
    if not current_ticket:
        return Command(update={"error_message": "No ticket information available"})
    ...
    if not final_response:
        return Command(update={"error_message": "No agent response found"})
    ...
    prompt = ChatPromptTemplate.from_messages(
        [("system", config.assessment_system_prompt), ("human", config.assessment_human_prompt)]
    )
    ...
    overall_confidence = min(state.get("overall_confidence", 1.0), assessment.confidence_score)
