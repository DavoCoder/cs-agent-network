...
Tests process_billing_ticket and should_continue functions with mocked external integrations.
"""
from unittest.mock import Mock, patch
from langchain_core.messages import HumanMessage, AIMessage, SystemMessage

from src.nodes.billing import process_billing_ticket, should_continue
...
class TestProcessBillingTicket:
    ...
    def test_processes_billing_ticket_with_user_message(self, mock_load_model):
        ...
        mock_load_model.assert_called_once_with(
            ...
            temperature=config.billing_temperature
        )
    ...
    def test_processes_billing_ticket_with_custom_config(self, mock_load_model):
        ...
        custom_config = Configuration(
            billing_model="openai/o1",
            billing_temperature=0.5
        )
        runtime = Mock()
    ...
    def test_processes_billing_ticket_falls_back_to_default_config(self, mock_load_model):
        ...
        mock_load_model.assert_called_once_with(
            default_config.billing_model,
            temperature=default_config.billing_temperature
        )
...
class TestShouldContinue:
    ...
    def test_routes_to_billing_tools_when_tool_calls_present(self):
        """Test that should_continue routes to billing_tools when tool calls are detected."""
    ...
    def test_routes_to_assessment_when_no_tool_calls(self):
        """Test that should_continue routes to assessment when no tool calls."""
    ...
    def test_routes_to_assessment_when_empty_messages(self):
        """Test that should_continue routes to assessment when messages are empty."""
        # Setup state with empty messages
        state: ConversationState = {
            "messages": []
        }
    ...
    def test_routes_to_assessment_when_messages_is_none(self):
        """Test that should_continue routes to assessment when messages is None."""
    ...
    def test_routes_to_billing_tools_with_multiple_tool_calls(self):
        """Test that should_continue routes to billing_tools with multiple tool calls."""
    ...
    def test_routes_to_assessment_after_tool_response(self):
        """Test that should_continue routes to assessment after receiving tool response."""
    ...
    def test_routes_correctly_with_conversation_history(self):
        """Test that should_continue routes correctly with conversation history."""
...
class TestBillingNodeIntegration:
    ...
    def test_full_flow_with_tool_call(self, mock_retrieve_kb, mock_load_model):
        ...
        mock_retrieve_kb.return_value = "--- Knowledge Base Articles ---\n\nArticle 1:\nContent: Pro plan costs $99/month\n"
        ...
        routing_result = should_continue(updated_state)
